...
where
    K: Hash + ?Sized + PartialEq + Eq + ToOwned,
    V: ToOwned + ?Sized,
{
    ...
    V: ToOwned + ?Sized,
{
    ...
    pub fn insert_owned_borrowed_key(
        ...
        self.inner
            .insert(Cow::Borrowed(key), Cow::Owned(value))
            .map(|x| x.into_owned())
    }
    ...
    pub fn insert_borrowed_owned_key(
        ...
        self.inner
            .insert(Cow::Owned(key), Cow::Borrowed(value))
            .map(|x| x.into_owned())
    }
    ...
    pub fn make_owned(&mut self, key: &K) -> Option<&V> {
        ...
        match val {
            ...
            Cow::Owned(_) => self.inner.get(key).map(|v| v.as_ref()),
        }
    }
    ...
    pub fn borrow_fields(&'a self) -> Self {
        let collection: HashMap<Cow<'a, K>, Cow<'a, V>> = self
            .inner
            .iter()
            .map(|(k, v)| match (k, v) {
                (Cow::Owned(key), Cow::Owned(val)) => (
                    Cow::Borrowed((*key).borrow()),
                    Cow::Borrowed((*val).borrow()),
                ),
